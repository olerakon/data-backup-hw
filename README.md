...
Комбинация full backup + инкрементные (incremental). Full — раз в сутки/неделю, инкрементные — ежечасно (только изменения с предыдущего бэкапа).​

1.3. Моментальное переключение на другую базу
Да, возможно через репликацию (master-slave или maaster-master). При поломке мастера слейв становится новым мастером за секунды (failover).
​
---
